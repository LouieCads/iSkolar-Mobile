...
export default function ScholarshipDetailsPage() {
  ...
  const onEdit = () => {
    if (!id) return;
    router.push({ pathname: `/scholarship/${id}/edit` } as any);
  };
  ...
  const onChangeImage = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0] && id) {
        const upload = await scholarshipService.uploadScholarshipImage(String(id), result.assets[0].uri);
        if (upload.success) {
          setScholarship((prev: any) => ({ ...prev, image_url: upload.image_url }));
        } else {
          Alert.alert('Upload failed', upload.message);
        }
      }
    } catch (e) {
